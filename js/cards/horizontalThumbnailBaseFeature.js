Vue.component('horizontalthumbnailbasefeature', {
  template: `
  <div class="flex-shrink-0 w-4/5 overflow-hidden bg-white card-radius-all dark:bg-gray-900">
    <div class="aspect-w-8 aspect-h-5 relative" v-if="post.articleData.images.length">
      ...
      <img class="object-cover card-radius-all" :src="post.articleData.images[0].rotator"/>
    </div>
    <div class="px-0 pt-4 pb-3 dark:bg-gray-900">
      <p class="text-sm pb-1 text-gray-600 dark:text-gray-500">{{ post.articleData.section.name ? post.articleData.section.name : '预设段落标题' }}</p>
